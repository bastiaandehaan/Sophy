...
def test_risk_management_integration(self, mock_setup):
    ...
    assert (
        len(check_trade_risk_calls) > 0
    ), "Risk manager check_trade_risk moet worden aangeroepen"
    ...
    ), "Geen signaal verwacht bij overschreden dagelijkse verlieslimiet"
...
def test_multiple_symbols_handling(self, mock_setup):
    ...
    # Verificatie is contextafhankelijk van de gegenereerde data  # In een echte test zouden we verschillende symbolen kunnen mappen naar verschillende testdata
