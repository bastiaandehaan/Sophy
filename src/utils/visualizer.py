...
class Visualizer:
    ...
    def plot_equity_curve(self, include_drawdown=True):
        ...
        for _, row in status_df.iterrows():
            ...
            if pd.notna(balance) and balance != "":
                ...
                balance_str = (
                    comment.split("Balance: ")[1].split(",")[0]
            ...
            equity_str = (
                comment.split("Equity: ")[1].split(",")[0]
                if "Equity: " in comment
                else None
            )
        ...
        equity_df = pd.DataFrame({"timestamp": timestamps, "balance": balances})
        ...
        initial_balance = (
            equity_df["balance"].iloc[0] if not equity_df.empty else 100000
        )
        final_balance = (
            equity_df["balance"].iloc[-1] if not equity_df.empty else initial_balance
        )
        total_return = ((final_balance / initial_balance) - 1) * 100
        max_drawdown = (
            equity_df["drawdown"].min() if "drawdown" in equity_df.columns else 0
        )
        ...
        axes[1].set_ylim(
            min(equity_df["drawdown"].min() * 1.2, -12), 1
        )  # Zorg voor goede y-as limieten
        axes[1].grid(True)
        ...
        plt.close(fig)  # Expliciet figuur sluiten
    ...
    def plot_trade_results(self):
        ...
        fig, axes = plt.subplots(
            num_symbols, 1, figsize=(16, 6 * num_symbols), squeeze=False
        )
        ...
        for i, symbol in enumerate(symbols):
            ...
            current_ax = axes[i, 0]  # Expliciet de huidige as selecteren
            ...
            if not buys.empty:
                ...
                if "Volume" in buys.columns:
                    sizes = (
                        buys["Volume"] * 50 + 50
                    )  # Schaal volume voor marker grootte
                    current_ax.scatter(
                        buys["Timestamp"],
                        buys["Price"],
                        s=sizes,
            ...
            for pair in paired_trades:
                if len(pair) == 2:  # Alleen complete trade paren
                    ...
                    mid_time = (
                        buy["Timestamp"] + (sell["Timestamp"] - buy["Timestamp"]) / 2
                    )
            ...
            # Formateer plot
            current_ax.set_title(f"Trades voor {symbol}", fontsize=16)
            current_ax.set_ylabel("Prijs", fontsize=14)

            # Voeg gridlines toe
            current_ax.grid(True)
            current_ax.legend(loc="upper left", fontsize=12)
            ...
            # Formateer x-as
            current_ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d %H:%M"))
        ...
        # Stel algemene x-label in voor hele figuur
        fig.text(0.5, 0.04, "Tijd", fontsize=14, ha="center")
    ...
    def plot_performance_summary(self):
        ...
        for symbol in trade_df["Symbol"].unique():
            ...
            for pair in pairs:
                if len(pair) == 2:  # Alleen complete trades
                    ...
                    if pd.isna(buy.get("Price", None)) or pd.isna(
                        sell.get("Price", None)
                    ):
                        continue  # Sla ongeldige trades over
            ...
            profit_factor = (
                abs(total_profit_pct / total_loss_pct)
                if total_loss_pct < 0
                else float("inf")
            )
        ...
        # 1. Win/Loss Ratio per Symbol (Bar chart)
        ...
        if not symbols:
            print("Geen symbolen beschikbaar voor performance analyse")
            plt.close(fig)
            return None
        ...
        ax2.bar(
            x - width / 2, avg_wins, width, label="Avg Win %", color="green", alpha=0.7
        )
        ax2.bar(
            x + width / 2, avg_losses, width, label="Avg Loss %", color="red", alpha=0.7
        )
        ...
        output_path = os.path.join(
            self.output_dir, f"performance_summary_{timestamp}.png"
        )
        plt.savefig(output_path, dpi=150)
        plt.close(fig)  # Expliciet figuur sluiten
