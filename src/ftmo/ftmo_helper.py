...
class FTMOHelper:
    ...
    def check_ftmo_compliance(self, initial_balance: float) -> Dict:
        ...
        if status_df.empty:
            return {
                "compliant": False,
                "reason": "No account status data available",
                "details": {},
            }
        ...
        if daily_status.empty:
            return {
                "compliant": False,
                "reason": "Insufficient balance data available",
                "details": {},
            }
        ...
        if not profit_target_met:
            reasons.append(
                f"Profit target not reached: {total_pnl_pct:.2f}% "
        ...
        if not daily_loss_compliant:
            ...
            reasons.append(
                f"Daily loss limit exceeded: {worst_day['daily_drawdown']:.2f}% on {worst_day['Date']}"
            )
        if not total_loss_compliant:
            reasons.append(
                f"Maximum drawdown exceeded: {max_drawdown:.2f}% "
                f"(limit: {self.ftmo_rules['max_total_loss'] * 100}%)"
            )
        if not trading_days_compliant:
            reasons.append(
                f"Insufficient trading days: {unique_trading_days} "
        ...
        details = {
            ...
            "daily_stats": daily_status.to_dict(orient="records"),
            # Converted for better JSON compatibility
        }
    ...
    def generate_trading_report(self, initial_balance: float) -> bool:
        ...
        try:
            ...
            plt.plot(
                daily_status["Date"],
                daily_status["close_balance"],
                marker="o",
                label="Balance",
            )
            plt.fill_between(
                daily_status["Date"],
                daily_status["min_balance"],
                daily_status["max_balance"],
                alpha=0.3,
                color="gray",
                label="Daily Range",
            )
            plt.axhline(
                y=initial_balance, color="r", linestyle="--", label="Initial Balance"
            )
